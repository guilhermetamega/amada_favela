...
serve(async (req) => {
  ...
  if (req.method !== "POST") return json(405, { error: "Método não permitido.", debugStep: "method-check" });

  const requestId = crypto.randomUUID();

  try {
    log("start", { requestId, origin: req.headers.get("origin"), userAgent: req.headers.get("user-agent") });
    ...
    log("payload-received", {
      requestId,
      raffleId,
      selectedCount: Array.isArray(selectedNumbers) ? selectedNumbers.length : -1,
    ...
    if (!raffleId || !Array.isArray(selectedNumbers) || selectedNumbers.length === 0 || !buyerName || !buyerPhone || !safeEmail) {
      return json(400, { error: "Dados inválidos para pagamento. Nome, telefone e e-mail são obrigatórios.", debugStep: "validate-input", requestId });
    }
    ...
    if (!supabaseUrl || !serviceRole || !mpToken) {
      return json(500, { error: "Secrets não configurados.", debugStep: "validate-secrets", requestId });
    }
    ...
    if (raffleError || !raffle) {
      log("raffle-load-failed", { requestId, raffleError: raffleError?.message });
      return json(404, { error: "Rifa não encontrada.", debugStep: "load-raffle", requestId });
    }

    if (raffle.status !== "active" || new Date(raffle.sales_end_at).getTime() < Date.now()) {
      return json(400, { error: "Rifa encerrada.", debugStep: "validate-raffle-status", requestId });
    }
    ...
    const mpResponse = await fetch("https://api.mercadopago.com/v1/payments", {
      ...
      headers: {
        ...
        "X-Idempotency-Key": `raffle-${raffleId}-${safeEmail}-${selectedNumbers.join("-")}`.slice(0, 64),
      },
      body: JSON.stringify({
    ...
    log("mercadopago-response", { requestId, status: mpResponse.status, body: mpData });
    ...
    if (!mpResponse.ok) {
      ...
      return json(400, { error: `Falha ao criar pagamento PIX.${cause ? ` ${cause}` : ""}`, details: mpData, debugStep: "mercadopago-create-payment", requestId });
    }
    ...
    return new Response(JSON.stringify({ error: error instanceof Error ? error.message : "Erro interno.", debugStep: "unhandled-catch", requestId }), { status: 500, headers: corsHeaders });
  }
});
