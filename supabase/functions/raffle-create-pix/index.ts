...
serve(async (req) => {
  ...
  try {
    ...
    const { data: raffle, error: raffleError } = await admin.from("sponsor_raffles").select("id,title,number_price_cents,sales_end_at,status").eq("id", raffleId).single();
    if (raffleError || !raffle) return json(404, { error: "Rifa não encontrada." });
    if (raffle.status !== "active" || new Date(raffle.sales_end_at).getTime() < Date.now()) return json(400, { error: "Rifa encerrada." });
