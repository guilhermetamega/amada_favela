...
async function sendRafflePurchaseEmail(params: { to: string; raffleTitle: string; numbers: number[]; buyerName: string; }) {
  const apiKey = Deno.env.get("RESEND_API_KEY");
  const from = Deno.env.get("RAFFLE_EMAIL_FROM") ?? "Rifas <no-reply@amadafavela.com.br>";
  if (!apiKey) return;
...
serve(async (req) => {
  ...
  if (!fetched || !payment) return new Response("payment not found", { status: 200 });

  if (payment.status === "approved") {
    const raffleId = payment.metadata?.raffle_id;
    const selectedNumbers = Array.isArray(payment.metadata?.selected_numbers) ? payment.metadata.selected_numbers : [];
    ...
    const buyerEmail = typeof payment.metadata?.buyer_email === "string" ? payment.metadata.buyer_email : "";
    const buyerName = typeof payment.metadata?.buyer_name === "string" ? payment.metadata.buyer_name : "Comprador";

    if (buyerEmail) {
      await sendRafflePurchaseEmail({
        to: buyerEmail,
        raffleTitle: raffle?.title ?? "Rifa",
        numbers: selectedNumbers.map((n: unknown) => Number(n)).filter((n: number) => Number.isInteger(n)),
        buyerName,
      });
    }
  }
