...
import { buildMercadoPagoAuthorizationUrl, getMercadoPagoConfig } from "../_shared/mercadopago.ts";
...
serve(async (req) => {
  ...
  try {
    ...
    if (!supabaseUrl || !serviceRole) return json(500, { error: "Secrets não configurados." });
    ...
    const { error } = await admin.from("sponsor_mercadopago_oauth_states").insert({
      state,
