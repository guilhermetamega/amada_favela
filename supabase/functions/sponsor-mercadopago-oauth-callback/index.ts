...
serve(async (req) => {
  ...
  if (!supabaseUrl || !serviceRole)
    return new Response("Secrets não configurados", { status: 500 });
  ...
  if (err || !code || !state)
    return Response.redirect(
      `${redirectBase}?mercadopago=error&message=${encodeURIComponent(err || "Parâmetros inválidos")}`,
      302,
    );

  const { data: oauthState } = await admin
    .from("sponsor_mercadopago_oauth_states")
    .select("id,sponsor_id,expires_at,used_at")
    .eq("state", state)
    .maybeSingle();
  if (
    !oauthState ||
    ...
    return Response.redirect(
      `${redirectBase}?mercadopago=error&message=${encodeURIComponent("State OAuth inválido")}`,
      302,
    );
  }
  ...
  try {
    ...
    return Response.redirect(
      `${redirectBase}?mercadopago=error&message=${encodeURIComponent(error instanceof Error ? error.message : "Erro OAuth")}`,
      302,
    );
  }
});
