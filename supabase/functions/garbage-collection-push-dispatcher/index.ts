...
async function getGoogleAccessToken() {
  ...
  if (!clientEmail || !privateKey) {
    throw new Error("FIREBASE_CLIENT_EMAIL/FIREBASE_PRIVATE_KEY não configurados.");
  }
...
async function sendFcmMessage(accessToken: string, projectId: string, token: string, schedule: ScheduleRow) {
...
serve(async (req) => {
  ...
  const timeZone = Deno.env.get("GARBAGE_COLLECTION_TIMEZONE") || "America/Sao_Paulo";
  ...
  try {
    ...
    const target = getZonedParts(new Date(Date.now() + 10 * 60 * 1000), timeZone);
    ...
    const schedules = ((schedulesData ?? []) as ScheduleRow[]).filter((schedule) =>
      schedule.pass_time.startsWith(target.hhmm),
    );

    if (!schedules.length) {
      return json(200, { checked_for: target, dispatched: 0 });
    }
    ...
    for (const schedule of schedules) {
      ...
      for (const token of tokens) {
        const result = await sendFcmMessage(accessToken, firebaseProjectId, token.fcm_token, schedule);
        if (result.ok) {
          scheduleSuccess += 1;
        } else {
          scheduleFailed += 1;
          if (result.details.includes("UNREGISTERED") || result.details.includes("INVALID_ARGUMENT")) {
            await admin
              .from("user_push_tokens")
    ...
    return json(200, { checked_for: target, dispatched, success, failed });
  } catch (error) {
    return json(500, {
