...
export default function ScheduleWeekTable({ items, canManage = false, onEdit, onDelete }: Props) {
      ...
      <table className="w-full text-sm">
        ...
        <tbody>
          {grouped.map(({ weekday, items: dayItems }) => (
            <tr key={weekday} className="border-t border-zinc-200 align-top dark:border-zinc-800">
              <td className="px-4 py-3 font-medium">{LABELS[weekday]}</td>
              <td className="px-4 py-3">
                {dayItems.length ? (
                  <div className="flex flex-wrap gap-2">
                    {dayItems.map((item) => (
                      <span key={item.id} className="rounded-full bg-emerald-100 px-2 py-1 text-xs font-semibold text-emerald-700 dark:bg-emerald-900/50 dark:text-emerald-300">
                        {item.pass_time.slice(0, 5)} {item.is_active ? "" : "(inativo)"}
                      </span>
                    ))}
                  </div>
                ) : (
                  <span className="text-zinc-400">Sem horários</span>
                )}
              </td>
              {canManage ? (
                <td className="px-4 py-3">
                  <div className="flex flex-wrap justify-end gap-2">
                    {dayItems.map((item) => (
                      <div key={`${item.id}-actions`} className="flex gap-1">
                        <button onClick={() => onEdit?.(item)} className="rounded-lg border border-zinc-300 p-1.5 text-zinc-600 hover:bg-zinc-100 dark:border-zinc-700 dark:text-zinc-300 dark:hover:bg-zinc-800" type="button"><Pencil size={14} /></button>
                        <button onClick={() => onDelete?.(item)} className="rounded-lg border border-rose-300 p-1.5 text-rose-600 hover:bg-rose-50 dark:border-rose-900 dark:hover:bg-rose-950" type="button"><Trash2 size={14} /></button>
                      </div>
                    ))}
                  </div>
                </td>
