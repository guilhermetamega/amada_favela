...
import { createHomeRentItem } from "@/services/supabase/home_rent";
import type { HomeRentItem } from "@/types/home_rent";

type Props = {
  open: boolean;
  onClose: () => void;
  onSaved: (item: HomeRentItem) => void;
  communityName: string;
  initialItem?: HomeRentItem | null;
};
...
export default function HomeRentCreateEditModal({
  open,
  onClose,
  onSaved,
  communityName,
  ...
  const formId = "home-rent-create-edit-form";
  ...
  async function handleSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    if (!form.address.trim()) {
      setErrorMessage("Informe um endereço.");
      return;
    }
    ...
    try {
      ...
      const created = await createHomeRentItem({
      ...
      setErrorMessage(
        error instanceof Error ? error.message : "Erro ao salvar anúncio.",
      );
    } finally {
      setLoading(false);
    }
    ...
    >
      ...
      >
        ...
        <div className="min-h-0 flex-1 overflow-y-auto px-5 py-5 overscroll-contain">
          ...
          <div className="mb-4 rounded-2xl border border-sky-200 bg-sky-50/80 px-4 py-3 dark:border-sky-900/50 dark:bg-sky-500/10">
            <p className="mt-1 text-xs text-sky-600 dark:text-sky-400">
              Esse anúncio será publicado automaticamente na comunidade do seu
              perfil.
            </p>
          </div>
          ...
          >
            <div className="md:col-span-2">
              ...
              <input
                ...
                className="h-12 w-full rounded-2xl border border-zinc-200 bg-zinc-50 px-4 text-zinc-900 outline-none transition focus:border-sky-400 dark:border-zinc-700 dark:bg-zinc-950 dark:text-white dark:focus:border-sky-500"
                required
              />
            </div>
            ...
            <div className="md:col-span-2">
              ...
              <textarea
                ...
                onChange={handleChange}
                rows={5}
                className="w-full rounded-2xl border border-zinc-200 bg-zinc-50 px-4 py-3 text-zinc-900 outline-none transition focus:border-sky-400 dark:border-zinc-700 dark:bg-zinc-950 dark:text-white dark:focus:border-sky-500"
                required
              />
            </div>
            ...
            <div>
              ...
              <input
                ...
                className="block w-full text-sm text-zinc-700 file:mr-3 file:rounded-xl file:border-0 file:bg-sky-500 file:px-4 file:py-2 file:font-medium file:text-white dark:text-zinc-300 dark:file:bg-sky-500 dark:file:text-white"
                required={!isEditMode}
              />
            </div>
        ...
        >
          <div className="flex justify-end gap-3">
            ...
            <button
              ...
              form={formId}
              disabled={loading}
              className="inline-flex h-11 items-center justify-center rounded-2xl bg-sky-500 px-4 text-sm font-semibold text-white transition hover:bg-sky-600 disabled:opacity-60"
            >
              {loading
                ? "Salvando..."
                : isEditMode
