import type { LostAndFoundFiltersState } from "@/types/lost_and_found";

type Props = {
  value: LostAndFoundFiltersState;
  onChange: (next: LostAndFoundFiltersState) => void;
};

export default function LostAndFoundFilters({ value, onChange }: Props) {
  return (
      ...
      <div className="grid grid-cols-1 gap-3 lg:grid-cols-[1.4fr_0.8fr_0.8fr]">
        ...
        <select
          ...
          onChange={(event) =>
            onChange({
              ...value,
              type: event.target.value as LostAndFoundFiltersState["type"],
            })
          }
          className="h-12 rounded-2xl border border-zinc-200 bg-zinc-50 px-4 text-sm text-zinc-900 outline-none transition focus:border-zinc-400 dark:border-zinc-700 dark:bg-zinc-950 dark:text-white"
        >
        ...
        <select
          ...
          onChange={(event) =>
            onChange({
              ...value,
              status: event.target.value as LostAndFoundFiltersState["status"],
            })
          }
          className="h-12 rounded-2xl border border-zinc-200 bg-zinc-50 px-4 text-sm text-zinc-900 outline-none transition focus:border-zinc-400 dark:border-zinc-700 dark:bg-zinc-950 dark:text-white"
        >
