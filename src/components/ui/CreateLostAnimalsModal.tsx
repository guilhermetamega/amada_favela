...
type CreateLostAndFoundModalProps = {
  isOpen: boolean;
  onClose: () => void;
  onCreated: (item: LostAnimalsItem) => void;
};
...
export default function CreateLostAnimalsModal({
  isOpen,
  onClose,
  onCreated,
  ...
  async function handleSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      const createdItem = await createLostAnimalsItem({
        name: form.name,
        ...
        <div className="mb-6 flex items-start justify-between gap-4">
          <div>
            ...
            <p className="mt-1 text-sm text-zinc-400">
              Cadastre um animal achado ou perdido.
            </p>
          </div>
