...
type CreateHomeRentModalProps = {
  isOpen: boolean;
  onClose: () => void;
  onCreated: (item: HomeRentItem) => void;
};
...
export default function CreateHomeRentModal({
  isOpen,
  onClose,
  onCreated,
}: CreateHomeRentModalProps) {
  ...
  async function handleSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      const createdItem = await createHomeRentItem({
        title: form.title,
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 px-4 py-6">
      <div className="max-h-[90vh] w-full max-w-2xl overflow-y-auto rounded-2xl border border-zinc-800 bg-zinc-900 p-6 shadow-2xl">
        <div className="mb-6 flex items-start justify-between gap-4">
          <div>
            <h2 className="text-2xl font-bold text-white">Nova casa</h2>
            <p className="mt-1 text-sm text-zinc-400">
              Cadastre uma casa para alugar ou vender.
            </p>
          </div>
