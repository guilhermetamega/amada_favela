...
export default function GarbageSchedulesPage() {
  ...
  const [editing, setEditing] = useState<GarbageCollectionSchedule | null>(
    null,
  );
  ...
  useEffect(() => {
    void getEditableGarbageSchedules()
      .then(setItems)
      .catch(() => undefined);
  }, []);
  ...
  async function onSubmit(e: React.FormEvent) {
    e.preventDefault();
    if (editing) {
      const updated = await updateGarbageSchedule(editing.id, {
        weekday,
        pass_time: passTime,
    ...
    const created = await createGarbageSchedule({
      weekday,
      pass_time: passTime,
    });
      ...
      <MainLayout className="space-y-4">
        <div className="rounded-3xl border border-zinc-200 bg-white p-5 shadow-sm dark:border-zinc-800 dark:bg-zinc-900">
          ...
          <p className="mt-1 text-sm text-zinc-500">
            Cadastre, edite e exclua os horários da coleta para sua comunidade.
          </p>
          <form
            onSubmit={onSubmit}
          ...
          >
            ...
            <button
              ...
              type="submit"
            >
              {isEditing ? "Salvar edição" : "Adicionar horário"}
            </button>
            {isEditing ? (
              <button
