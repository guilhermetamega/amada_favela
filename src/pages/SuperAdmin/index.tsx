...
import PlatformFeesSection from "@/components/superAdmin/PlataformFeesSection";
...
export default function SuperAdminPage() {
  ...
  useEffect(() => {
    ...
    async function load() {
      try {
        ...
        const [authResult, usersData] = await Promise.all([
  ...
  const stripeButtonLabel = openingStripePartner
    ? "Abrindo Stripe..."
  ...
  const addAddressItemRow = () => {
    setAddressItemsForm((current) => [...current, createAddressItemForm()]);
  };

  const removeAddressItemRow = (index: number) => {
    setAddressItemsForm((current) =>
      current.length === 1
        ? current
    ...
    <DashboardLayout>
      <MainLayout>
        <div className="mx-auto max-w-6xl space-y-6">
          <DashboardHeader
            title="Super Admin"
            description="Gerencie usuários globais, comunidades, taxas e contas integradas da plataforma."
          />
            ...
            <div className="grid gap-5 lg:grid-cols-[1fr_auto] lg:items-center">
              <div className="space-y-3">
                {loadingStripePartnerStatus ? (
                  ...
                  <>
                    {stripePartnerStatus?.stripe_onboarding_completed ? (
                      <div className="flex items-start gap-2 rounded-2xl border border-emerald-500/30 bg-emerald-500/10 p-4 text-sm text-emerald-700 dark:text-emerald-200">
              ...
              <button
                ...
                className="inline-flex w-full items-center justify-center gap-2 rounded-2xl border border-violet-400/40 bg-violet-600 px-5 py-3 text-sm font-semibold text-white transition hover:bg-violet-500 disabled:cursor-not-allowed disabled:opacity-60 lg:w-auto"
              >
          ...
          <PlatformFeesSection
            id="platform-fees"
            isOpen={openSectionId === "platform-fees"}
            onToggle={toggleSection}
          />
          ...
          <UsersSection
            ...
            onUserCommunityChange={(userId, community) => {
              void handleCommunityChange(userId, community);
            }}
          />
            ...
            {loadingCommunities ? (
              ...
              <div className="grid grid-cols-1 divide-y divide-zinc-200 dark:divide-zinc-800">
                {communities.map((item) => (
                  <div
                    key={item.key}
                    className="flex flex-col gap-3 p-4 text-left md:flex-row md:items-center md:justify-between"
                  >
                    <div>
                    ...
                    <button
                      ...
                      className="rounded-xl border border-zinc-300 px-4 py-2 text-sm font-semibold text-zinc-700 transition hover:bg-zinc-100 dark:border-zinc-700 dark:text-zinc-100 dark:hover:bg-zinc-800"
                    >
                      Editar
                    </button>
                  </div>
            ...
            <div className="max-h-[90vh] w-full max-w-3xl overflow-y-auto rounded-2xl border border-zinc-800 bg-zinc-900 p-5">
              ...
              <div className="mt-4">
                <div className="mb-2 flex items-center justify-between gap-2">
                  ...
                  <button
                    ...
                    className="inline-flex items-center gap-1 rounded-lg border border-zinc-700 px-3 py-1.5 text-xs text-zinc-100 transition hover:bg-zinc-800"
                  >
                    <Plus size={13} />
                    Adicionar linha
                  </button>
                  ...
                  {addressItemsForm.map((item, index) => (
                    ...
                    >
                      ...
                      <button
                        ...
                        className="inline-flex items-center justify-center rounded-lg border border-red-500/40 px-2 py-2 text-red-300 transition hover:bg-red-500/10"
              ...
              <div className="mt-5 flex flex-col gap-2 sm:flex-row sm:justify-end">
                <button
                  ...
                  className="rounded-xl border border-zinc-700 px-4 py-2 text-zinc-200 transition hover:bg-zinc-800 disabled:opacity-60"
                >
                  Cancelar
                </button>
                ...
                <button
                  type="button"
                  onClick={() => {
                    void handleSubmitCommunity();
                  }}
                  disabled={savingCommunity}
                  className="rounded-xl bg-violet-600 px-4 py-2 font-semibold text-white transition hover:bg-violet-500 disabled:opacity-60"
                >
