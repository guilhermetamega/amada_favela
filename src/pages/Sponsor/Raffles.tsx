...
export default function SponsorRafflesPage() {
  ...
  async function load() {
    try {
      ...
      const onlyCurrentSponsor = data.filter((item) => item.sponsor_id === sponsorId);
  ...
  async function openInsights(raffleId: string) {
    ...
    try {
      ...
      setErrorMessage(
        error instanceof Error ? error.message : "Erro ao carregar detalhes da rifa.",
      );
    } finally {
  ...
  return (
    <MainLayout className="min-h-dvh bg-zinc-50 px-4 py-6 text-zinc-900 dark:bg-zinc-950 dark:text-zinc-50 sm:py-8">
        ...
        <div className="mt-6 grid grid-cols-1 gap-6 xl:grid-cols-[1fr_0.95fr]">
          <section className="rounded-[28px] border border-zinc-200 bg-white p-6 shadow-sm dark:border-zinc-800 dark:bg-zinc-900 sm:p-8">
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              <button disabled={saving || !mpConnected} className="rounded-2xl bg-zinc-900 px-5 py-3 font-semibold text-white dark:bg-white dark:text-zinc-900">{saving ? "Salvando..." : "Criar rifa"}</button>
            </form>
          </section>
          ...
          <section className="rounded-[28px] border border-zinc-200 bg-white p-6 shadow-sm dark:border-zinc-800 dark:bg-zinc-900">
            ...
            {loading ? (
              ...
              <div className="mt-4 space-y-3">
                {items.map((it) => (
                  <div key={it.id} className="rounded-2xl border border-zinc-200 bg-zinc-50 p-3 text-sm dark:border-zinc-800 dark:bg-zinc-950">
                    <div className="flex items-center justify-between gap-3">
                      <span>{it.title}</span>
                      <button onClick={() => void openInsights(it.id)} className="text-xs font-semibold text-indigo-600 dark:text-indigo-300">Ver arrecadação</button>
                    </div>
                    <p className="mt-2">{window.location.origin}/raffles/{it.slug}</p>
                  </div>
                ))}
              </div>
            )}
      ...
      {insightsOpen ? (
        <div className="fixed inset-0 z-50 flex items-center justify-center overflow-y-auto bg-zinc-950/70 p-4" onClick={() => setInsightsOpen(false)}>
          <div className="w-full max-w-3xl rounded-3xl border border-zinc-200 bg-white p-4 dark:border-zinc-800 dark:bg-zinc-900 sm:p-6" onClick={(e) => e.stopPropagation()}>
            <div className="mb-4 flex items-center justify-between">
              <h3 className="text-lg font-bold">Resumo da rifa</h3>
              <button className="rounded-xl border px-3 py-1 text-sm" onClick={() => setInsightsOpen(false)}>Fechar</button>
            </div>
            {insightsLoading || !insights ? <p className="text-sm text-zinc-500">Carregando detalhes...</p> : <div><p className="text-2xl font-bold">R$ {(insights.total_raised_cents / 100).toFixed(2)}</p><p>Número vencedor: {insights.raffle.winning_number ?? "Pendente"}</p></div>}
          </div>
        </div>
      ) : null}
    </MainLayout>
