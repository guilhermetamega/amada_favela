...
export default function ResumeBuilderPage() {
  ...
  async function handleDownload() { if (!previewRef.current || !profile || downloading) return; try { setDownloading(true); setErrorMessage(""); const dataUrl = await toPng(previewRef.current, { cacheBust:true, pixelRatio:3, backgroundColor:"#ffffff" }); const imageBytes = await fetch(dataUrl).then((response)=>response.arrayBuffer()); const pdf = await PDFDocument.create(); const page = pdf.addPage([595.28, 841.89]); const image = await pdf.embedPng(imageBytes); const pageWidth = page.getWidth(); const pageHeight = page.getHeight(); page.drawImage(image, { x: 0, y: 0, width: pageWidth, height: pageHeight }); const pdfDataUri = await pdf.saveAsBase64({ dataUri: true }); const link = document.createElement("a"); link.href = pdfDataUri; link.download = `curriculo-${normalizeFileName(profile.fullname)}.pdf`; link.click(); } catch { setErrorMessage("Não foi possível gerar o PDF agora."); } finally { setDownloading(false); } }
  ...
  return <DashboardLayout><MainLayout><div className="mx-auto max-w-7xl"><DashboardHeader title="Criador de Currículos" />{errorMessage?<div className="mb-4 text-sm text-red-500">{errorMessage}</div>:null}{successMessage?<div className="mb-4 inline-flex items-center gap-2 text-emerald-600"><CheckCircle2 size={14}/>{successMessage}</div>:null}<div className="mb-4 flex gap-2"><button type="button" onClick={handleDownload} disabled={downloading} className="rounded-xl border px-4 py-2 text-sm">{downloading?<LoaderCircle size={14} className="mr-1 inline animate-spin"/>:<FileText size={14} className="mr-1 inline"/>}Baixar PDF</button><button type="button" onClick={()=>window.print()} className="rounded-xl border px-4 py-2 text-sm"><Download size={14} className="mr-1 inline"/>Imprimir</button></div><div className="grid grid-cols-1 gap-5 lg:grid-cols-[minmax(0,430px)_1fr]"><aside className="space-y-4"><PersonalDataSection profile={profile} associationAddress={associationAddress} email={formData.email} linkedin={formData.linkedin} lattes={formData.lattes} onEmailChange={(v)=>updateField("email",v)} onLinkedinChange={(v)=>updateField("linkedin",v)} onLattesChange={(v)=>updateField("lattes",v)} />
...
</aside><section className="rounded-[28px] border border-zinc-200 bg-zinc-100 p-3 dark:border-zinc-800 dark:bg-zinc-950 sm:p-5"><div className="rounded-3xl bg-zinc-200 p-2 dark:bg-zinc-900 sm:p-4"><div className="w-full"><ResumePreview profile={profile} associationAddress={associationAddress} data={formData} previewRef={previewRef} /></div></div></section></div></div></MainLayout></DashboardLayout>;
}
