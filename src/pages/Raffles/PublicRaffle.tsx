      ...
      <div className="mx-auto max-w-5xl rounded-[28px] border border-zinc-200 bg-white p-6 shadow-sm dark:border-zinc-800 dark:bg-zinc-900 sm:p-8">
        {loading ? "Carregando rifa..." : !raffle ? "Rifa não encontrada." : (
          ...
          <>
            ...
            {closed ? (
              <button className="mt-6 rounded-2xl border border-zinc-300 px-4 py-3 text-sm dark:border-zinc-700">Rifa encerrada. Número ganhador: {raffle.winning_number ?? "-"}</button>
            ) : (
              <>
                <div className="mt-6 flex items-center justify-between rounded-2xl border border-zinc-200 bg-zinc-50 p-3 text-xs dark:border-zinc-800 dark:bg-zinc-950">
                  ...
                  <div className="flex gap-2">
                    <button disabled={page <= 1} onClick={() => setPage((p) => p - 1)} className="rounded-xl border px-3 py-1 disabled:opacity-40">Anterior</button>
                    <button disabled={page >= pageCount} onClick={() => setPage((p) => p + 1)} className="rounded-xl border px-3 py-1 disabled:opacity-40">Próxima</button>
                  </div>
                </div>
